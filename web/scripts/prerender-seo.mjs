...
const faqDescription = seoConfig.faqDescription;
...
const data = JSON.parse(
  await readFile(path.join(publicDir, "free_models.json"), "utf8"),
);
const models = Array.isArray(data.models) ? data.models : [];
const archivedModels = Array.isArray(data.archivedModels)
  ? data.archivedModels
  : [];
...
const providerNameFor = (model) =>
  providerNames.get(model.providerId) || model.providerId || "OpenRouter";
  ...
  return {
    ...
    "@graph": [
      ...
      },
      {
        ...
        featureList: Array.isArray(model.supported_parameters)
        ...
        additionalProperty: [
          ...
          },
          {
            ...
            value: model.architecture?.modality || "text->text",
          },
          ...(archived
            ? [
                {
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 2,
            name: archived ? "Archive" : "Free models",
            item: canonicalUrl(archived ? "/archive" : "/"),
          },
          {
            "@type": "ListItem",
            position: 3,
...
function renderDocument(template, metadata, body) {
  return template
    ...
    .replace(/<script type="module"([^>]*)><\/script>/g, (tag, attributes) =>
      /\bdefer\b/.test(attributes)
        ? tag
        : `<script type="module"${attributes} defer></script>`,
    );
}
...
function sitemapXml() {
  ...
  for (const model of [
    ...models,
    ...archivedModels.map((entry) => entry.model),
  ].filter(Boolean))
    seen.add(modelPath(model.id));
...
await writeRoute(
  ...
  "/archive",
  {
    ...
    route: "/archive",
    structuredData: pageSchema(archiveTitle, archiveDescription, "/archive", [
      { name: siteName, path: "/" },
...
for (const model of models) {
  ...
  await writeRoute(
    ...
    route,
    {
      ...
      description: modelDescription(model, providerName),
      route,
      type: "article",
      structuredData: modelSchema(model),
    },
    modelBody(model, false),
  );
}
for (const entry of archivedModels) {
  ...
  await writeRoute(
    ...
    route,
    {
      ...
      route,
      type: "article",
      structuredData: modelSchema(model, true),
    },
