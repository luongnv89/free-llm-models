...
function historyEntryKey(providerId, modelId) {
  const p = String(providerId ?? DEFAULT_PROVIDER_ID);
  const m = String(modelId ?? '');
  if (p.includes(KEY_SEPARATOR) || m.includes(KEY_SEPARATOR)) {
    throw new Error(
      `history id must not contain U+0000: ${JSON.stringify([p, m])}`
    );
  }
  return `${p}${KEY_SEPARATOR}${m}`;
}
