...
describe("harness compatibility registry", () => {
  it.each(EXPECTED)(
    "records $providerId × $harnessId as $status",
    (expected) => {
      expect(
        COMPATIBILITY_REGISTRY[expected.harnessId][expected.providerId],
  ...
  it("has exactly one provenance-backed entry for all 32 combinations", () => {
    ...
    expect(
      entries.every((entry) => entry.providerSignupUrl.startsWith("https://")),
    ).toBe(true);
    expect(
      entries.every((entry) => entry.providerDocsUrl.startsWith("https://")),
    ).toBe(true);
  });
});
...
describe("generateHarnessRecipe", () => {
  it("maps NVIDIA for Pi and OpenCode while preserving the source model ID", () => {
    ...
    expect(
      pi.snippets.find((snippet) => snippet.id === "run")?.content,
    ).toContain("--provider 'nvidia'");
    expect(opencode).toMatchObject({
      sourceModelId: MODEL_ID,
      modelId: "nvidia/owner/model",
    });
    expect(
      opencode.snippets.find((snippet) => snippet.id === "run")?.content,
    ).toContain("'nvidia/owner/model'");
  });

  it("adds the OpenRouter namespace once for OpenCode nested IDs", () => {
    expect(
      generateHarnessRecipe("opencode", "openrouter", "anthropic/claude-3")
        .modelId,
  ...
  it("only advertises Claude Code for eligible Anthropic OpenRouter models", () => {
    expect(
      generateHarnessRecipe("claude-code", "openrouter", "anthropic/claude-3")
        .status,
    ).toBe("experimental");
    const unsupported = generateHarnessRecipe(
      "claude-code",
      "openrouter",
      "google/gemini-2",
    );
  ...
  it("returns an explicit unsupported result for unknown providers", () => {
    for (const harnessId of HARNESS_IDS) {
      const recipe = generateHarnessRecipe(
        harnessId,
        "future-provider",
        MODEL_ID,
      );
  ...
  it("renders ordered steps with unique copy targets and provenance", () => {
    for (const expected of EXPECTED) {
      const recipe = generateHarnessRecipe(
        expected.harnessId,
        expected.providerId,
        expected.harnessId === "claude-code" ? "anthropic/claude-3" : MODEL_ID,
      );
      expect(recipe.lastVerified).toBe("2026-08-27");
      expect(recipe.provenance?.harnessDocsUrl).toBe(recipe.docsUrl);
      expect(recipe.snippets).toEqual(
        recipe.steps.flatMap((step) => step.snippets),
      );
      expect(new Set(recipe.snippets.map((snippet) => snippet.id)).size).toBe(
        recipe.snippets.length,
      );
  ...
  it("uses environment references and mergeable Codex TOML without overwrite commands", () => {
    ...
    const config = codex.snippets.find(
      (snippet) => snippet.language === "toml",
    )?.content;
    expect(config).toContain('wire_api = "responses"');
    expect(config).toContain("[model_providers.openrouter]");
    expect(config).not.toMatch(/(^|\n)\s*(cat|tee)\s+.*[>]/);
    expect(
      codex.snippets.some((snippet) =>
        snippet.content.includes("$OPENROUTER_API_KEY"),
      ),
    ).toBe(true);
  });
  ...
  it("serializes hostile IDs safely and remains deterministic", () => {
    ...
    const json = openCode.snippets.find(
      (snippet) => snippet.language === "json",
    )?.content;
    expect(() => JSON.parse(json!)).not.toThrow();
    expect(
      codex.snippets.find((snippet) => snippet.id === "run")?.content,
    ).toContain("'\"'\"'");
    expect(
      [...openCode.snippets, ...codex.snippets].some((snippet) =>
        snippet.content.includes("sk-"),
      ),
    ).toBe(false);
    expect(generateHarnessRecipe("codex", "openrouter", hostile)).toEqual(
      codex,
    );
  });
});
