...
describe('CodeSnippets', () => {
  ...
  it('falls back to OpenRouter defaults when provider metadata is absent', async () => {
    ...
    expect(container.textContent).toContain('https://openrouter.ai/api/v1/chat/completions');
    expect(container.textContent).toContain('OPENROUTER_API_KEY');
    const keysLink = container.querySelector('a[href="https://openrouter.ai/keys"]');
  ...
  it('renders snippets with the selected model provider metadata', async () => {
    ...
    const keysLink = container.querySelector('a[href="https://console.acme.ai/api-keys"]');
