...
const PROVIDER_INFO: Record<
  ProviderId,
  Omit<RecipeProvenance, "harnessDocsUrl" | "verificationDate">
> = {
  openrouter: {
...
function unsupportedCaveat(
  pair: CompatibilityEntry,
  providerId: string,
): string[] {
  return [
    ...(pair.caveats ?? []),
...
function claudeCodeSteps(
  ...
  return [
    ...
    },
    {
      ...
      description:
        "Use Claude Code's built-in authentication to connect to the provider.",
      snippets: [
        {
          id: "login",
          label: "Start Claude Code and authenticate",
          language: "text",
          content: "claude",
        },
        {
          id: "login-prompt",
          label: "When prompted, enter provider URL and key",
          language: "text",
          content:
            "Enter API base URL: " +
  ...
  return [
    ...
    },
    {
      ...
      snippets: [
        {
          id: "login",
          label: "Open Pi provider login",
          language: "text",
          content: "/login",
        },
        {
          id: "model-picker",
          label: "Open the Pi model picker",
          language: "text",
          content: "/model",
        },
        {
...
function openCodeModelId(pair: CompatibilityEntry, modelId: string): string {
  const sourceModel =
    pair.providerId === "openrouter" && modelId.startsWith("openrouter/")
      ? modelId.slice("openrouter/".length)
      : modelId;
  return `${pair.harnessProviderId}/${sourceModel}`;
}

function opencodeSteps(
  pair: CompatibilityEntry,
  modelId: string,
): RecipeStep[] {
  const envVar = API_KEY_ENV[pair.providerId];
  ...
  return [
    ...
    },
    {
      ...
      snippets: [
        {
          id: "connect",
          label: "Start Codex and configure provider",
          language: "text",
          content: "codex --model " + modelId,
        },
        {
          id: "connect-prompt",
          label: "When prompted, enter provider details",
          language: "text",
          content:
            "Enter API base URL: " +
            PROVIDER_BASE_URL[pair.providerId] +
            "\nEnter API key: " +
            "$" +
            envVar,
        },
...
function supportsClaudeCode(providerId: string, modelId: string): boolean {
  const sourceModel = modelId.startsWith("openrouter/")
    ? modelId.slice("openrouter/".length)
    : modelId;
  return (
    providerId === "openrouter" &&
    /^anthropic\/claude(?:[-/]|$)/.test(sourceModel)
  );
}
...
export function generateHarnessRecipe(
  ...
  const modelSupported =
    ...
    supportsClaudeCode(providerId, sourceModelId);
  const status = modelSupported ? pair.status : "unsupported";
  const caveats = modelSupported
    ? pair.caveats
    : unsupportedCaveat(pair, providerId);
  const steps =
    status === "unsupported"
      ? []
      : harnessId === "claude-code"
  ...
  const copyAllSafe =
    ...
    status !== "unsupported" &&
    snippets.length > 0 &&
    snippets.every((snippet) => snippet.language === "shell");
  ...
  return {
    ...
    modelId:
      harnessId === "opencode"
        ? openCodeModelId(pair, sourceModelId)
        : sourceModelId,
    steps,
    snippets,
    copyAll: copyAllSafe
    ...
    ...(pair.minimumKnownVersion
      ? { minimumKnownVersion: pair.minimumKnownVersion }
      : {}),
