...
describe("getProvider", () => {
  ...
  it("returns the whole id when there is no slash", () => {
    expect(getProvider(makeModel({ id: "orphan", name: "Orphan" }))).toBe(
      "orphan",
    );
  });

  it("falls back to Unknown when the id is empty", () => {
    expect(getProvider(makeModel({ id: "", name: "Anonymous" }))).toBe(
      "Unknown",
    );
  });
});
...
describe("isNewModel", () => {
  ...
  it("returns false when addedToFreeList is missing even if created is recent", () => {
    ...
    expect(isNewModel(makeModel({ id: "a/b", name: "A", created: nowS }))).toBe(
      false,
    );
  });
});
...
describe("filterAndSortModels", () => {
  ...
  it("intersects the source filter with search and other filters", () => {
    ...
    expect(ids({ sources: ["or"], search: "llama" })).toEqual(["meta/llama-b"]);
...
describe("getProviderQuickFilterOptions", () => {
  ...
  it("keeps every required provider visible when no models are loaded", () => {
    expect(
      getProviderQuickFilterOptions([]).map(({ id, count }) => ({ id, count })),
    ).toEqual([
