...
async function render(
  providerMeta: ProviderMetadata = PROVIDER,
  providerId?: string,
  modelId = MODEL_ID,
) {
  ...
  await act(async () => {
    root!.render(
      createElement(HarnessSetupGuide, { modelId, providerMeta, providerId }),
    );
  });
}
...
describe("HarnessSetupGuide", () => {
  ...
  it("renders four accessible tabs and one visible associated panel", async () => {
    ...
    expect(
      container.querySelectorAll('[role="tabpanel"][hidden]'),
    ).toHaveLength(3);
  });
  ...
  it("shows provenance links, verification date, and the access warning", async () => {
    ...
    expect(
      container.querySelector(
        'a[href="https://code.claude.com/docs/en/llm-gateway"]',
      ),
    ).toBeTruthy();
    expect(
      container.querySelector('a[href="https://openrouter.ai/docs"]'),
    ).toBeTruthy();
    expect(
      container.querySelector('a[href="https://openrouter.ai/keys"]'),
    ).toBeTruthy();
    ...
    expect(container.textContent).toContain(
      "prompts or tool output leave your machine",
    );
  ...
  it("switches panels and renders numbered, copyable supported steps", async () => {
    await render();
    const piTab =
      container.querySelector<HTMLButtonElement>("#harness-tab-pi")!;
  ...
  it("only shows Copy all for safe shell concatenation", async () => {
    ...
    expect(
      container.querySelector("#harness-panel-claude-code button")?.textContent,
    ).toContain("Copy");
    // Claude Code now has text snippets in the login step, so no Copy all
    expect(
      container.querySelector(
        '#harness-panel-claude-code button[aria-label="Copy all"]',
      ),
    ).toBeNull();
    await act(async () =>
      container.querySelector<HTMLButtonElement>("#harness-tab-pi")!.click(),
    );
    expect(
      container.querySelector(
        '#harness-panel-pi button[aria-label="Copy all"]',
      ),
    ).toBeNull();
  });

  it("explains unsupported combinations without rendering setup commands", async () => {
    const google: ProviderMetadata = {
      ...PROVIDER,
      id: "google",
  ...
  it("reports a manual-copy fallback when clipboard and execCommand fail", async () => {
    ...
    const copyButton = container.querySelector<HTMLButtonElement>(
      '#harness-panel-claude-code button[aria-label="Copy Check OPENROUTER_API_KEY"]',
    )!;
