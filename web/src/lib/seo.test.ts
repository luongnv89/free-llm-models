...
describe('SEO helpers', () => {
  ...
  it('provides a useful model description when source data is short', () => {
    ...
    expect(modelSeoDescription(model, 'Acme AI').length).toBeGreaterThanOrEqual(50);
  });

  it('creates a de-duplicated item list schema for the model directory', () => {
    const schema = buildHomeStructuredData([model, { ...model, providerId: 'other' }]);
    ...
    expect(itemList?.itemListElement).toEqual([
      expect.objectContaining({ name: 'Example Model', url: expect.stringContaining('/model/') }),
    ]);
  });
  ...
  it('escapes closing markup characters in JSON-LD payloads', () => {
    expect(serializeStructuredData({ description: '</script>' })).toContain('\\u003c/script>');
  });
});
