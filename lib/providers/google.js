...
function createGoogleAdapter({
  ...
  const metadata = defineProviderMetadata({
    ...
    // The native baseUrl above only accepts generateContent-style requests;
    // client snippets use Google's OpenAI-compatible endpoint instead.
  ...
  return {
    ...
    normalize(raw) {
      ...
      return {
        id,
        // Honor displayName when present; when re-normalizing a canonical
        // model (no displayName), keep its existing `name` so normalization
        // is truly idempotent. Entries lacking any identity never get
        // "repaired" by re-normalization.
        name:
